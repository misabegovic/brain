...
markdown + git, directly compatible with any markdown-native
editor). The three genuinely Claude-Code-specific pieces and their
...
Standing ideas from studying prior art, implemented, plus the
...
brain's own nested agent: a competitive-positioning page (since
removed for the public repo) and the market-readiness-gaps
...
Deliberately not adopted from the studied prior art: a vector
