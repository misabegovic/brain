...
| `/attention`        | Groom the **attention board** — the daily ranked shortlist at `wiki/_state/attention/` of what deserves the operator's day, built from connector snapshots and the inbox. Mechanical half is `brain.py attention refresh`; the skill owns tier, category, why, command and the score's components. Capped and discardable — cards leave by being dismissed, unlike queue items, which leave by being done. Renders at `wiki/_views/attention.md`. | `attention` skill. |
...
| "what should I work on?" / "the board" / "morning briefing" / "groom the board" | `/attention`                                                |
