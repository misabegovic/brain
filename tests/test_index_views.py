...
def test_compile_block_shorthands():
    sql, params = brain._compile_block(
        {"pages": {"kind": "decision", "status": "accepted"}})
    assert "kind = ?" in sql and params == ["decision", "accepted"]
    sql, params = brain._compile_block({"state": "deadlines"})
    assert "surface = ?" in sql and params == ["deadlines"]
    sql, params = brain._compile_block({"inbox": {"kind": "research"}})
    assert "kind = ?" in sql and params == ["research"]
...
def test_custom_views_render_from_specs(index):
    ...
    assert "Auto-generated by `brain.py views`" in engineer
