...
3. **Pre-work checks** per `AGENTS.md` § Working inside a sibling repo:
   ...
   - **Blast radius before touching a shared symbol.** `python3 tools/brain.py enola impact <symbol>` reports fan-in, fan-out and named callers from the on-disk fact set, so the size of a change is read rather than estimated — a symbol with hundreds of callers turns a small edit into a different conversation about scope. `python3 tools/brain.py structure findings --repo <repo>` is the always-available fallback. Both skip cleanly when absent. And read the decision trail of every file about to change: `python3 tools/brain.py enola govern <path>` names the compiled pages whose anchors cover it — the pattern-fit scan starts from the decisions that shaped the area, not just the adjacent code, and a change that contradicts a governing page is a **stop and ask**, not a silent divergence.
   - **Grade the architecture, and read the grade yourself.** Before editing a repo, `python3 tools/brain.py enola baseline pin <repo>` freezes the "before"; before pushing, `enola check <repo>` grades what the change did to it. **It reports and never blocks** — the graph is the ceiling, never the floor, so a verdict is for you to weigh beside the test result. `declined — snapshots not comparable` means the delta describes how the snapshots were built rather than what you edited: read it as *not asked*, never as a pass. Skips cleanly when the binary or cluster is absent.
