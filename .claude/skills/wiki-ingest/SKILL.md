...
| External URL whose content is **competitor-shaped** | Save to `sources/web/competitors/<competitor>/<slug>--<shortid>.md`. The folder-per-competitor shape mirrors the wiki-side `wiki/org/competitors/<competitor>/`. Frontmatter carries `url:`, `pulled: YYYY-MM-DD`, `fetched_via: <tool>`, `source_kind: <press-release \| product-page \| trade-press \| ...>`. See [`wiki/brain/adrs/competitor-intel-ingestion.md`](../../../wiki/brain/adrs/competitor-intel-ingestion.md). |
