...
| Spoke client      | ✅ `tools/brain-agent.py` | The harness-side half: an agent in any harness runs it to `emit` / `pull` / `subscribe` / `listen` against a hosted brain, mirroring the server's signing. Config via `BRAIN_URL` / `BRAIN_AGENT_ID` / `BRAIN_AGENT_SECRET`. The brain is the hub; the harness stays the execution spoke. |
